...
namespace Eiffel.Messaging.Core
{
    public class Mediator : IMediator
    {
        ...
        internal Mediator(IServiceProvider serviceProvider, List<IMessagingMiddleware> middlewares) : this(serviceProvider)
        {
            _messagingMiddleware = new MessagingMiddleware(middlewares);
        }
        ...
        private Task DispatchMessageAsync<TCommand>(TCommand command, CancellationToken cancellationToken)
            where TCommand : ICommand, IMessage
        {
        ...
        private async Task<TReply> DispatchMessageAsync<TReply>(IQuery<TReply> query, CancellationToken cancellationToken)
            where TReply : class
        {
            ...
            return await _messagingMiddleware.Handle(handler, query, cancellationToken);
        }
